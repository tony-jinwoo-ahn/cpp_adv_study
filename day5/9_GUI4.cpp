...
public:
	...
	// 아래 가상함수에서 처리할 메시지를 담은 배열을 반환해야 한다.
...
public:
	virtual MSG_ENTRY* GetMessageEntry() override
	{
		...
		static MSG_ENTRY arr[] = {
			{WM_LBUTTONDOWN, 
			static_cast<HANDLER>(&MyWindow::LButtonDown)
			},
			{0, 0} 
		};
		return arr;
	...
	void LButtonDown() { std::cout << "MyWindow LButton" << std::endl; }
	void KeyDown() { std::cout << "MyWindow KeyDown" << std::endl; }
};
